...
const stickySidebar = () => {
  ...
  const labels = [...document.querySelectorAll('.Typical__label')];
  const blocks = [...document.querySelectorAll('.Typical__container')];

  labels.forEach(label => label.onclick = () => {
    blocks.forEach(block => block.classList.remove('Typical__active'));
